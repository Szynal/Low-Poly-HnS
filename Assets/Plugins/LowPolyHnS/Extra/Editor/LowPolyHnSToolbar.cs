...
{
    public static class LowPolyHnSToolbar
    {
        private enum BTN_POS
        {
            L,
            M,
            R
        ...
        public class Item
        {
            ...
            public Texture2D GetTexture()
            {
                if (texture == null)
                {
                    texture = AssetDatabase.LoadAssetAtPath<Texture2D>(iconPath);
                }
        ...
        public const string PATH_ICONS = "Assets/Plugins/LowPolyHnS/Extra/Icons/Toolbar/{0}";

        private const string KEY_TOOLBAR_ENABLED = "LowPolyHnS-toolbar-enabled";
        ...
        private static float MOVE_OFFSET_X;
        private static float MOVE_OFFSET_Y;

        private static bool STYLES_INITIALIZED;
        ...
        [MenuItem("LowPolyHnS/Tools/Show Toolbar %&t", true)]
        public static bool ShowToolbarValidate()
        {
            return !IsEnabled();
        }

        [MenuItem("LowPolyHnS/Tools/Show Toolbar %&t", false)]
        ...
        private static void RegisterDefaultItems()
        {
            ...
            RegisterItem("close.png", "Close the Toolbar", Disable, 998);
        ...
        private static void OnPaintToolbar(SceneView sceneview)
        {
            ...
            if (registeredItem) ITEMS.Sort((x, y) => x.priority.CompareTo(y.priority));
            ...
            if (DRAGGING)
            {
                Vector2 delta = Event.current.mousePosition - MOUSE_POSITION;
